...
from ..exceptions.error_error_error_62_exception import ErrorErrorError62Exception
...
class ProvisioningController(BaseController):
    ...
    def delete_subscription(self):
        ...
        if _context.response.status_code == 400:
            raise ErrorErrorError62Exception('Invalid or missing request parameters', _context)
        elif _context.response.status_code == 401:
            raise ErrorErrorError62Exception('Invalid or no credentials passed in the request', _context)
        elif _context.response.status_code == 403:
            raise ErrorErrorError62Exception('Authorization credentials passed and accepted but account does not have permission', _context)
        elif _context.response.status_code == 404:
            raise ErrorErrorError62Exception('The requested URI does not exist', _context)
        elif (_context.response.status_code < 200) or (_context.response.status_code > 208): 
            raise ErrorErrorError62Exception('An internal error occurred when processing the request', _context)
